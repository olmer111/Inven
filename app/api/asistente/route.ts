...
export async function POST(request: Request) {
  ...
  if (proveedor === "openrouter") {
    ...
    if (!clave) {
      return NextResponse.json(
        { error: "Necesitas una clave API de OpenRouter. Configúrala en Configuración → Modelo de IA, o añade OPENROUTER_API_KEY en el servidor." },
        { status: 503 }
      );
    }
    return usarOAICompat(mensajes, systemPrompt, modelo, "https://openrouter.ai/api/v1", clave, {
